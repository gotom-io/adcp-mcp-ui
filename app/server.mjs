...
8. All three booking tools (accept_proposal, buy_products, legacy create_media_buy) answer with a TASK, not a finished buy: { status: "submitted", task_id, ext: { gotom_io: { media_buy_id, campaign_link } } }. goTom creates the campaign as an offer and a goTom user has to approve it. Always display the task_id, the media_buy_id and the campaign_link — render the link as a real markdown link on its own line, for example [Open the campaign in goTom](<the url>) — never as bare text, never truncated, never invented when absent. Tell the user the campaign is waiting for approval in goTom. To see whether it was approved, call get_task_status with that task_id and include_result true: status "completed" carries the finished buy in result (media_buy_id, packages or purchase_bindings with package_id, confirmed_at); status "failed" with error.code OFFER_NOT_BOOKED means goTom dropped the offer — the user must book again. Never claim a buy is confirmed while the task is still submitted. Every task read (get_task_status, list_tasks) repeats ext.gotom_io.media_buy_id and campaign_link, so the campaign is always identifiable. The package_ids for the creative step do not wait for approval: get_media_buys with the media_buy_id from ext works on an offer too and returns packages[].package_id. The ad tags reach the campaign one of two ways, decided BEFORE you book:
...
proposal_id and proposal_terms_digest are the COMMITTED successor's (from refine_proposals), not the draft's. A wrong digest answers PROPOSAL_NOT_FOUND — use exactly the digest you displayed. Returns the submitted task envelope of rule 8: { status: "submitted", task_id, ext: { gotom_io: { media_buy_id, campaign_link } } }. Once goTom approved the offer, get_task_status (include_result true) returns in result: { media_buy_id, media_buy_status, confirmed_at, accepted_proposal, purchase_bindings: [{ purchase_index, product_id, package_id }], ext: { gotom_io: { display_result_link_to_open_in_browser } } }. media_buy_status is pending_creatives until the ad tags are delivered via sync_creatives — the package_ids for that come from get_media_buys or from purchase_bindings. campaign_link and display_result_link_to_open_in_browser are the same campaign confirmation page: display it as a markdown link (rule 8).
...
The response is the submitted task envelope of rule 8 — { status: "submitted", task_id, ext: { gotom_io: { media_buy_id, campaign_link } } } — never the buy itself. The finished buy arrives on the task's result once goTom approved the offer: media_buy_status there is always pending_creatives, even when inline creatives were stored — call get_media_buys for the real status. That result may carry ext.gotom_io.inline_creative_warnings listing creatives that could NOT be stored; those, and only those, still need a sync_creatives call. ext.gotom_io.campaign_link on the envelope (and ext.gotom_io.display_result_link_to_open_in_browser on the result) is the campaign confirmation page — display it as a markdown link (rule 8).
...
Returns { task_id, status, ext: { gotom_io: { media_buy_id, campaign_link } }, result?, error? }. status "submitted" = the offer is still waiting for approval in goTom; "completed" = approved, result holds the finished buy (rule 8); "failed" with error.code OFFER_NOT_BOOKED = goTom dropped or deleted the offer, book again if still wanted. list_tasks (params: account only) lists every task of the account with the same ext, without result.
