...
proposal_id and proposal_terms_digest are the COMMITTED successor's (from refine_proposals), not the draft's. A wrong digest answers PROPOSAL_NOT_FOUND — use exactly the digest you displayed. Returns { media_buy_id, media_buy_status, accepted_proposal, purchase_bindings: [{ purchase_index, product_id, package_id }], ext: { display_result_link_to_open_in_browser } }. media_buy_status is pending_creatives until the ad tags are delivered via sync_creatives — the package_ids for that come from purchase_bindings. ext.display_result_link_to_open_in_browser is the campaign confirmation page: display it as a markdown link (rule 8).
