...
The response is the submitted task envelope of rule 8 — { status: "submitted", task_id, ext: { gotom_io: { media_buy_id, campaign_link, note } } } — never the buy itself. The finished buy arrives on the task's result once goTom approved the offer: media_buy_status there is always pending_creatives, even when inline creatives were stored — call get_media_buys for the real status. That result may carry ext.gotom_io.inline_creative_warnings listing creatives that could NOT be stored; those, and only those, still need a sync_creatives call. ext.gotom_io.campaign_link on the envelope (and ext.gotom_io.display_result_link_to_open_in_browser on the result) is the campaign confirmation page — display it as a markdown link (rule 8).
